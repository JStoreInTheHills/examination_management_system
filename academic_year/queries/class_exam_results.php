...
$sql = "SELECT exam_name, (SELECT SUM(marks) FROM result) as marks FROM class_exams ce JOIN year y ON y.year_id = ce.year_id 
        JOIN tblclasses c ON ce.class_id = c.id
        JOIN exam e ON e.exam_id = ce.exam_id
        WHERE year_name =:academic_year AND ClassName like :class_name";
