...
$all_students;  // Number of students in the class. 
...
// $subjects_name = Array(); // array to hold the subject names. 

// Query to fetch the number of students in the class. 
try {
    
    // Distict Students count from the tblstudents. 
    $students_count = "SELECT COUNT(DISTINCT students_id) AS StudentCount 
                       FROM result 
                       WHERE class_id=:class_id 
                       AND class_exam_id =:class_exam_id";

    $query = $dbh->prepare($students_count);
    ...
    foreach($student_count as $students){
        // Place all the students count in the variable
        $all_students = $students["StudentCount"]; 
    }
}catch(Exception $e){
...
try{
        ...
        $pdf->Cell(94, 8, " Total Number of Results Declared: " .$all_students. " " ,$border=1,$ln=0,'C',$fill=true, $link='', $stretch=0, $ignore_min_height=true, $calign='T', $valign='B');
        ...
        $pdf->Cell(0, 8, "Printed By: ". $_SESSION['alogin']. " ",$border=1,$ln=1,'C',$fill=true, $link='', $stretch=0, $ignore_min_height=true, $calign='T', $valign='B');
...
try{
    $total_marks_query = "SELECT COUNT(DISTINCT students_id) as sc, SUM(marks) as m
                            FROM result WHERE class_id =:class_id 
                            AND class_exam_id =:class_exam_id";


    $total_marks_ = $dbh->prepare($total_marks_query);
    $total_marks_->bindParam(":class_id", $class_id, PDO::PARAM_STR);
    $total_marks_->bindParam(":class_exam_id", $class_exam_id, PDO::PARAM_STR);

    $total_marks_->execute();
    $total_results = $total_marks_->fetchAll();

    $total_marks__;
    $total_students_sat__ = 0;

    foreach($total_results as $item){
        $total_marks__ = $item['m'];
        $total_students_sat__ = $item['sc'];
    }
...
try{
    ...
    $pdf->Cell($width_cell[10], 7, $grades,$border=1,$ln=1,'C',$fill=true, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='R');  
    ...
    $pdf->Cell(83, 7, "Mean Score / Average",$border=1,$ln=0,'C',$fill=true, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='R');  
       ...
       for($i = 0; $i < count($subjectMarksArray); $i++){
           ...
           $avgSubjects  = round($subjectMarksArray[$i] / $all_students, 1);
